...
public class JsonUtil {
    ...
    /**
     * 将json字符串解码为ArrayList，泛型为指定的class
     *
     * @param <T>
     * @param json
     * @param t
    ...
    public static <T> ArrayList<T> toArrayList(String json, Class<T> classzz) {
        try {
            return (ArrayList<T>) MAPPER.readValue(json, getCollectionType(ArrayList.class, classzz));
        } catch (Exception e) {
            throw new RuntimeException("error decode json to" + classzz, e);
        }
    }

    private static JavaType getCollectionType(Class<?> collectionClass, Class<?> classzz) {
        return MAPPER.getTypeFactory().constructParametricType(collectionClass, classzz);
    }
}